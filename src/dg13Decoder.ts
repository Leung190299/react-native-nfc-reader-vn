...
export const DG13_LABELS: Record<keyof Omit<DG13Data, 'rawFields'>, string> = {
  identityNo: 'Số CCCD',
  fullName: 'Họ và tên',
...
const TAG_MAP: Record<string, DG13Field> = {
  '01': 'fullName',
  '02': 'identityNo',
  '03': 'dateOfBirth',
  '04': 'gender',
...
export class DG13Decoder {
  ...
  static decodeBytes(bytes: Uint8Array): DG13Data {
    const result: DG13Data = {rawFields: {}};
  ...
  private static parseFlat(bytes: Uint8Array, result: DG13Data): void {
    let pos = 0;
    let iterations = 0;
    ...
    while (pos < bytes.length - 1 && iterations < 200) {
      ...
      const {length, newPos} = this.parseLength(bytes, pos);
      pos = newPos;
  ...
  private static splitBySeparator(str: string, sep: string): string[] {
    const result: string[] = [];
    let current = '';
  ...
  private static extractFromSequence(bytes: Uint8Array, result: DG13Data): void {
  ...
  private static parseFieldsByOrder(parts: string[], result: DG13Data): void {
    ...
    for (let i = 0; i < cleanParts.length; i++) {
      ...
      if (
        p.toLowerCase().includes('sẹo') ||
        p.toLowerCase().includes('nốt ruồi') ||
        p.toLowerCase().includes('vết')
      ) {
  ...
  private static parseLength(
    bytes: Uint8Array,
    pos: number,
  ): {length: number; newPos: number} {
    if (pos >= bytes.length) return {length: 0, newPos: pos};
    let length = bytes[pos];
    let newPos = pos + 1;
    if (length > 0x80) {
      const lengthBytesCount = length & 0x7f;
      length = 0;
      if (newPos + lengthBytesCount > bytes.length)
        return {length: 0, newPos: bytes.length};
      for (let i = 0; i < lengthBytesCount; i++) {
        length = (length << 8) | bytes[newPos + i];
      }
      newPos += lengthBytesCount;
    } else if (length === 0x80) {
      return {length: 0, newPos: newPos};
    }
